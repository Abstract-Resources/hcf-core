...
final class DemoteArgument extends Argument {
    ...
    public function onPlayerExecute(Player $sender, Profile $profile, string $label, array $args): void {
        if (count($args) === 0) {
            $sender->sendMessage(TextFormat::RED . 'Please use /' . $label . ' demote <player>');
        ...
        if ($factionMember->getRole() !== ProfileData::OFFICER_ROLE) {
            $sender->sendMessage(TextFormat::RED . 'You cannot demote this member!');
        ...
        $faction->registerMember($factionMember->getXuid(), $factionMember->getName(), ProfileData::MEMBER_ROLE, $factionMember->getKills());

        $faction->broadcastMessage(ServerUtils::replacePlaceholders('FACTION_MEMBER_DEMOTED', [
